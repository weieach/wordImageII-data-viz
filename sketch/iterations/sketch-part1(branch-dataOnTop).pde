...
void draw() {
  ...
  //draw circle (on top of everything else)
  for (int i = startingYear; i < yearsLB.length; i++) {
    ...
    if (leafY[i]<0) {
      ...
      float val = round(leafY[i] * -100) / 100.0;
      String valStr = String.format("%.1f", val)
        .replaceAll("0+$", "")   // remove trailing zeros
        .replaceAll("\\.$", ""); // remove trailing dot if all decimals removed
      text(valStr, width/170, height/400);
      fill(indexBasedColor);
    } else {
      translate(x + gapFromCenter, -(i-startingYear)*gridSize);
      rotate(-PI/2);
